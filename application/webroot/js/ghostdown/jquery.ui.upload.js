            ...
            complete: function (result) {
                ...
                function preLoadImage() {
                    ...
                    $progress.animate({"opacity": 0}, 250, function () {
                        $dropzone.find('span.media').after('<img class="fileupload-loading" src="' + service.url + '/img/loading.gif" />');
                        if (!settings.editor) {$progress.find('.fileupload-loading').css({"top": "56px"}); }
                    });
                    $dropzone.trigger("uploadsuccess", [result.uploadimage[0].url]);
                    $img.one('load', function () {
                        animateDropzone($img);
                    }).attr('src', result.uploadimage[0].url);
                }
                ...
                $dropzone.find('.js-fileupload').fileupload().fileupload("option", {
                    ...
                    fail: function (e, data) {
                        ...
                        if (data.jqXHR.status === 413) {
                            $dropzone.find('div.js-fail').text("이미지 용량이 너무 큽니다.");
                        } else if (data.jqXHR.status === 415) {
                            $dropzone.find('div.js-fail').text("지원하지 않는 이미지 포맷입니다. 가능한 이미지 포맷 : .PNG, .JPG, .GIF, .SVG.");
                        } else {
                            $dropzone.find('div.js-fail').text("뭔가 잘못되었네요 :(");
                        }
